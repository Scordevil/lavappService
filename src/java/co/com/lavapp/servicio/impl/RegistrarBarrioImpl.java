...
import co.com.lavapp.servicio.RegistrarBarrio;
 ...
 */
@Stateless
@Path("/registrarBarrio")
public class RegistrarBarrioImpl implements RegistrarBarrio{

    @GET
    @Produces({"application/json"})
    @Override
    public Barrio_TO registrarBarrio(@QueryParam("nombre") String nombre, 
            @QueryParam("idLocalidad") int idLocalidad, 
            @QueryParam("idZona") int idZona, 
            @QueryParam("idEstrato") int idEstrato) throws Exception {
        Barrio_TO barrio = new Barrio_TO(nombre, new Localidad_TO(idLocalidad), new Zona_TO(idZona), new Estrato_TO(idEstrato));
        BarrioDAO barrioDao = new BarrioDAOImpl();
        return barrioDao.registrarBarrio(barrio);
    }    
}
